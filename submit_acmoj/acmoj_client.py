...
class ACMOJClient:
    ...
    def submit_code(self, problem_id: int, code: str, language: str = "cpp") -> Optional[Dict]:
        """Submit raw source code content (used for single-file problems like src.hpp)."""
...
def main():
    ...
    # File submission sub-command (single-file source code)
    submit_file_parser = subparsers.add_parser("submit-file", help="Submit single source file content")
    submit_file_parser.add_argument("--problem-id", type=int, required=True, help="Problem ID")
    submit_file_parser.add_argument("--file", type=str, required=True, help="Path to the source file (e.g., src.hpp)")
    ...
    if args.command == "submit":
        ...
        try:
            ...
            return
        result = client.submit_code(args.problem_id, code, getattr(args, 'language', 'cpp'))
